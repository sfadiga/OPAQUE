...
class TextWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.text_edit.setPlainText("This is a text editor widget.\nYou can type here...")
...
class CounterWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        increment_btn.clicked.connect(lambda: self.spin_box.setValue(self.spin_box.value() + 1))
        controls_layout.addWidget(increment_btn)
        
        decrement_btn = QPushButton("-1")
        decrement_btn.clicked.connect(lambda: self.spin_box.setValue(self.spin_box.value() - 1))
        controls_layout.addWidget(decrement_btn)
        
        layout.addLayout(controls_layout)
        
        # Add some info
        info_label = QLabel("This is a counter widget with workspace save/load support.")
...
class MainWindow(QMainWindow):
    ...
    def _setup_menu(self):
        ...
        add_text_action.triggered.connect(lambda: self.tab_widget.add_tab("Text Editor", self._create_text_widget()))
        tab_menu.addAction(add_text_action)
        
        add_counter_action = QAction("Add Counter", self)
        add_counter_action.triggered.connect(lambda: self.tab_widget.add_tab("Counter", self._create_counter_widget()))
        tab_menu.addAction(add_counter_action)
        
        add_list_action = QAction("Add List", self)
        add_list_action.triggered.connect(lambda: self.tab_widget.add_tab("List", self._create_list_widget()))
    ...
    def _on_tab_renamed(self, index, old_name, new_name):
        ...
        self.statusBar().showMessage(f"Renamed tab at index {index}: '{old_name}' -> '{new_name}'")
    ...
    def _save_workspace(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.information(self, "Success", f"Workspace saved successfully to:\n{file_path}")
        ...
        if file_path:
            try:
                ...
                if success:
                    ...
                    QMessageBox.information(self, "Success", f"Workspace loaded successfully from:\n{file_path}")
                else:
                    QMessageBox.warning(self, "Warning", "Workspace loaded with some errors. Check the console for details.")

...
class DataViewerModel(BaseModel):
    ...
    auto_refresh = settings_field(default_value=True, description="Auto-refresh data on startup")
    refresh_interval = settings_field(default_value=30, description="Refresh interval in seconds")
    max_items = settings_field(default_value=100, description="Maximum items to display")
    
    # Workspace state that saves with workspace
    last_filter = workspace_field(default_value="", description="Last applied filter")
    sort_column = workspace_field(default_value="id", description="Column to sort by")
    sort_order = workspace_field(default_value="asc", description="Sort order")
    
    def initialize(self):
        """Initialize the model."""
        # Create observable properties
        self.create_property('data', [])
        self.create_property('filters', {})
        self.create_property('selected_item', None)
    ...
    def add_item(self, item: Dict[str, Any]):
        ...
        if len(self.data) >= self.max_items:
            self.notify("error", f"Maximum items limit ({self.max_items}) reached")
            return
    ...
    def sort_data(self, column: str, order: str = "asc"):
        ...
        try:
            reverse = (order == "desc")
            self.data = sorted(self.data, key=lambda x: x.get(column, ''), reverse=reverse)
            self.notify("data", self.data)
